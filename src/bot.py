...
async def send_message3(message, user_message):
    await message.response.defer(ephemeral=isPrivate)
    try:
        response = '> **' + user_message + '** - <@' + \
            str(message.user.id) + '>\n\n'
        response = f"{response}{await responses.handle_response3(user_message)}"
        if len(response) > 1900:
            # Split the response into smaller chunks of no more than 1900 characters each(Discord limit is 2000 per chunk)
            if "```" in response:
                # Split the response if the code block exists
...
def run_discord_bot():
    ...
    async def chat(interaction: discord.Interaction, *, message: str):
        if int(interaction.user.id) == int(config['discord_admin']):
            ...
            user_message = message
            channel = str(interaction.channel)
            await send_message3(interaction, user_message)
            import os
            # get config.json path
            config_dir = os.path.abspath(__file__ + "/../../")
            config_name2 = 'curusage.txt'
